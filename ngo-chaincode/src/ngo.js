...
let Chaincode = class {
  ...
  */
  async createAsset(stub, args) {
    ...
    console.log('##### create asset arguments: ' + JSON.stringify(args));
    ...
    if (assetQuery.toString()) {
      throw new Error('##### createAsset - This asset already exists: ' + json['id']);
    }
  ...
  async updateAsset(stub, args) {
    console.log('============ START : update Asset =============')
    // args is passed as a JSON string
    let json = JSON.parse(args);
    let key = json['id'];
    let assetQuery = await stub.getState(key);

    if (!assetQuery || assetQuery.length === 0) {
      throw new Error(`Asset ${key} does not exist`);
    }

    let asset = JSON.parse(assetQuery.toString());
  ...
  async getAsset(stub, args) {
    ...
    const asset = JSON.parse(assetQuery.toString());
  ...
  async transferAsset(stub, args) {
    let json = JSON.parse(args);
    let id = json['id'];
    let newOwner = json['new_owner']
    const assetBytes = await stub.getState(id);
    ...
    console.log(`Asset ${id} transferred to new owner: ${newOwner}`);
  ...
  async getAssetHistory(stub, args) {
    let json = JSON.parse(args);
    let id = json['id'];
    const historyIterator = await stub.getHistoryForKey(id);
    const assetHistory = [];

    while (true) {
        const history = await historyIterator.next();
        if (history.value && history.value.value.toString()) {
            const asset = JSON.parse(history.value.value.toString());
    ...
    return shim.success(Buffer.from(JSON.stringify(assetHistory)));
  }


  async getAssetReceipt(stub, args) {

  }


  async getAssetStats(stub, args) {
